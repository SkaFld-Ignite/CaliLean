...
export function registerSyncCommand(program: Command): void {
  program
    .command("sync")
    .description("Sync local config to a Medusa instance (the main orchestrator)")
    .action(async () => {
      ...
      // 6. Products — need categoryMap first
      ...
      // 8. API Keys — need salesChannelMap first
      ...
      log.header("Summary")
      log.info(
        `Created: ${totals.created}  Updated: ${totals.updated}  Skipped: ${totals.skipped}  Errors: ${totals.errors}`
      )
      if (dryRun) {
